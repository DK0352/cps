cps compares and syncronizes two directories by copying only the missing files and directories, but also enables you to overwrite files of different size or different last modification time, as well as to copy or delete any surplus data. It has extremely useful statistical output which tells you the exact size of data to be copied before actually commencing the copying operation, as well as number and size of files and directories, surplus files, etc. Before any writing operation begins, cps can lists all the files and directories that are about to be copied, and ask you for the confirmation. It also allows you to create a text file with all the files and directories that are about to be copied without actually copying anything, as well as creating a text file with the complete file and directory content of both directories. You can also copy or delete surplus files and directories without synchronizing the source and destination. 
...
1. To copy missing files and directories from directory1 (or source) to directory2 (or destination), you don't have to supply any options. For example:

```
cps /home/user/Documents/photos/2023 /mnt/backup/photos/2023
```
When you are asked if you want to copy missing files and directories, answer yes to begin copying operation or no if you want to quit. Use the -q option if you don't want to be asked for the confirmation. The program doesn't differentiate
whether the pathnames end with a '/' or not. Whether you type /mnt/backup/photos/2023 or /mnt/backup/photos/2023/ - it is the same.
...
3. To copy missing files and directories from /home/user/Documents/dir1 (source) to /home/user/Documents/dir2 (destination) and to delete files and directories from /home/user/Documents/dir2 (destination) that don't exist in 
/home/user/Documents/dir1 (source):
```
...
```
Such files are identified in the statistical output of the program even during the default (without any options) operation of the program:
```
...
```
To not update the last access time on files during reading operation use --no-access-time or -a
```
cps -a /home/user/Documents/dir1 /home/user/Documents/dir2
```
...
```
8. If you are not entirely sure what needs or what will be copied, you can just print out the list of files that will be copied, without actually coping anything. It is also useful in case there are many files and directories listed out
...
cps -k /home/user/Documents/copy_list /home/user/Documents/dir1 /home/user/Documents/dir2
```
9. You can list complete content of files and directories from both directory1 (source) and directory2 (destination) without copying anything. use option --just-content-file=/path/to/file where /path/to/file is a desired pathname of your choice.
```
